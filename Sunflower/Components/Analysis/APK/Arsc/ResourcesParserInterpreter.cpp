...
std::vector<std::string> ResourcesParserInterpreter::getParserStringValue(const std::string& id) {
            ...
            if(nullptr != pEntry) {
                if(pEntry->flags & ResTable_entry::FLAG_COMPLEX) {
                    ...
                    for(int i = 0 ; i < pMapEntry->count ; i++) {
                        string name = mParser->getNameForResTableMap((pMap+i)->name);
                        if(!ResourcesParser::isTableMapForAttrDesc((pMap+i)->name)){
                            string value = mParser->valueToString(&(pMap+i)->value);
                    ...
                    if (ID_TYPE != type) {
                        string value = mParser->valueToString(pValue);
                        result.push_back(value);
                    }
                }
            }
